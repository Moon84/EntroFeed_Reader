# -*- coding: utf-8 -*-
"""Handler base classes for EntroFeed.

This module provides backward-compatible base classes.
New code should use src.plugins.llm.ModelWrapperBase and related base classes.
"""
...
# For backward compatibility, we define the same base classes here.
# The actual implementations now use plugins/llm/ModelWrapperBase etc.


class HandlerBase(BaseModel, ABC):
    """Base class for all handlers (backward compatible)."""

    id: ClassVar[str] = "generic handler base"


class ContentRetrievalHandler(HandlerBase):
    """Base class for content retrieval handlers."""
...
class LLMHandler(HandlerBase):
    """Base class for LLM handlers (backward compatible).

    Note: New code should use src.plugins.llm.ModelWrapperBase instead.
    """
