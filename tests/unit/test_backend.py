...
def setup(mocker: MockerFixture):
    ...
    with mocker.patch("src.constants.DATA_DIR", patch_dir):
        db = load_storage_config()
        backend = EntroFeedBackend(db)
        yield db, backend
...
def test_format_time():
    int_time = 1732670009
    str_time = EntroFeedBackend._format_time(int_time)

    # Check ISO 8601 format with timezone
    assert str_time == "2024-11-27T01:13:29+00:00"
