...
class TinyDBStorageHandler(StorageHandler):
    ...
    def insert_feed(self, feed: Feed):
        ...
        table.insert({"id": feed.id, "feed": feed.model_dump()})
    ...
    def update_feed_entry_state(
        ...
        table = self.db.table("entries")
        query = Query().id.matches(entry_id)
        results = table.search(query)
        if not results:
            return

        entry_data = results[0]["entry"]
        if is_read is not None:
            entry_data["is_read"] = is_read
            entry_data["read_at"] = int(__import__("time").time()) if is_read else None
        if liked is not None:
            entry_data["liked"] = liked
        if is_favorite is not None:
            entry_data["is_favorite"] = is_favorite

        table.upsert({"id": entry_id, "feed_id": results[0]["feed_id"], "entry": entry_data}, cond=query)
    ...
    async def upsert_entry_content(self, content: EntryContent):
        ...
        table.upsert(
            {"id": content.id, "entry_contents": content.model_dump()},
            cond=query,
        )
